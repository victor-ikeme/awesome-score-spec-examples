# React-Nginx-Score-Example — Score.dev Workload Example

This project demonstrates how to deploy the `react-nginx-score-example` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:react-nginx-score-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#react-nginx-score-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#react-nginx-score-example.dns --format '{{ .host }}'
```
