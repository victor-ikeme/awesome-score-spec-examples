# Spring-Postgres — Score.dev Workload Example

This project demonstrates how to deploy the `spring-postgres` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:spring-postgres:4200
```
...
```bash
score-compose resources get-outputs dns.default#spring-postgres.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#spring-postgres.dns --format '{{ .host }}'
```
