# Gitea-Postgres — Score.dev Workload Example

This project demonstrates how to deploy the `gitea-postgres` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:gitea-postgres:4200
```
...
```bash
score-compose resources get-outputs dns.default#gitea-postgres.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#gitea-postgres.dns --format '{{ .host }}'
```
