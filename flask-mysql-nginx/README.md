# Flask-Mysql-Nginx — Score.dev Workload Example

This project demonstrates how to deploy the `flask-mysql-nginx` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:flask-mysql-nginx:4200
```
...
```bash
score-compose resources get-outputs dns.default#flask-mysql-nginx.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#flask-mysql-nginx.dns --format '{{ .host }}'
```
