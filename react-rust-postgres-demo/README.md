# React-Rust-Postgres-Demo — Score.dev Workload Example

This project demonstrates how to deploy the `react-rust-postgres-demo` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:react-rust-postgres-demo:4200
```
...
```bash
score-compose resources get-outputs dns.default#react-rust-postgres-demo.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#react-rust-postgres-demo.dns --format '{{ .host }}'
```
