# Postgresql-Pgadmin-Score-Example — Score.dev Workload Example

This project demonstrates how to deploy the `postgresql-pgadmin-score-example` workload using [Score.dev](https://score.dev), via:
...
```
postgresql-pgadmin-score-example/
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:postgresql-pgadmin-score-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#postgresql-pgadmin-score-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#postgresql-pgadmin-score-example.dns --format '{{ .host }}'
```
