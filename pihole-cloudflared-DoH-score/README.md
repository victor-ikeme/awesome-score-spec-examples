# Pihole-Cloudflared-Doh-Score — Score.dev Workload Example

This project demonstrates how to deploy the `pihole-cloudflared-DoH-score` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:pihole-cloudflared-DoH-score:4200
```
...
```bash
score-compose resources get-outputs dns.default#pihole-cloudflared-DoH-score.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#pihole-cloudflared-DoH-score.dns --format '{{ .host }}'
```
