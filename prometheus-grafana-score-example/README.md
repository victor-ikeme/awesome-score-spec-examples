# Prometheus-Grafana-Score-Example — Score.dev Workload Example

This project demonstrates how to deploy the `prometheus-grafana-score-example` workload using [Score.dev](https://score.dev), via:
...
```
prometheus-grafana-score-example/
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:prometheus-grafana-score-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#prometheus-grafana-score-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#prometheus-grafana-score-example.dns --format '{{ .host }}'
```
