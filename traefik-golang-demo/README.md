# Traefik-Golang-Demo — Score.dev Workload Example

This project demonstrates how to deploy the `traefik-golang-demo` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:traefik-golang-demo:4200
```
...
```bash
score-compose resources get-outputs dns.default#traefik-golang-demo.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#traefik-golang-demo.dns --format '{{ .host }}'
```
