# Angular — Score.dev Workload Example

This project demonstrates how to deploy the `angular` workload using [Score.dev](https://score.dev), via:
...
```bash
score-compose resources get-outputs dns.default#angular.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#angular.dns --format '{{ .host }}'
```
