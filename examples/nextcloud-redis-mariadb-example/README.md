# Nextcloud-Redis-Mariadb-Example — Score.dev Workload Example

This project demonstrates how to deploy the `nextcloud-redis-mariadb-example` workload using [Score.dev](https://score.dev), via:
...
```
nextcloud-redis-mariadb-example/
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:nextcloud-redis-mariadb-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#nextcloud-redis-mariadb-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#nextcloud-redis-mariadb-example.dns --format '{{ .host }}'
```
