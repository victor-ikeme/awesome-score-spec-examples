# Nginx-Wsgi-Flask-Example — Score.dev Workload Example

This project demonstrates how to deploy the `nginx-wsgi-flask-example` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:nginx-wsgi-flask-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#nginx-wsgi-flask-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#nginx-wsgi-flask-example.dns --format '{{ .host }}'
```
