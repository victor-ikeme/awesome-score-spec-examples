# Wasmedge-Mysql-Nginx — Score.dev Workload Example

This project demonstrates how to deploy the `wasmedge-mysql-nginx` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:wasmedge-mysql-nginx:4200
```
...
```bash
score-compose resources get-outputs dns.default#wasmedge-mysql-nginx.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#wasmedge-mysql-nginx.dns --format '{{ .host }}'
```
