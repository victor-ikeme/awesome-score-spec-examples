# Nginx-Aspnet-Mysql-Example — Score.dev Workload Example

This project demonstrates how to deploy the `nginx-aspnet-mysql-example` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:nginx-aspnet-mysql-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#nginx-aspnet-mysql-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#nginx-aspnet-mysql-example.dns --format '{{ .host }}'
```
