# Minecraft-Server-Score-Example — Score.dev Workload Example

This project demonstrates how to deploy the `minecraft-server-score-example` workload using [Score.dev](https://score.dev), via:
...
```
minecraft-server-score-example/
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:minecraft-server-score-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#minecraft-server-score-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#minecraft-server-score-example.dns --format '{{ .host }}'
```
