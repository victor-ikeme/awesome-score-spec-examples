# Django-Postgres-Score-Example — Score.dev Workload Example

This project demonstrates how to deploy the `django-postgres-score-example` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:django-postgres-score-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#django-postgres-score-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#django-postgres-score-example.dns --format '{{ .host }}'
```
