# Nodejs-Redis-Nginx-Example — Score.dev Workload Example

This project demonstrates how to deploy the `nodejs-redis-nginx-example` workload using [Score.dev](https://score.dev), via:
...
score-compose generate score/score.yaml \
  --build '={"context":"","tags":[""]}' \
  --publish 4200:nodejs-redis-nginx-example:4200
```
...
```bash
score-compose resources get-outputs dns.default#nodejs-redis-nginx-example.dns --format '{{ .host }}'
```
...
```bash
score-k8s resources get-outputs dns.default#nodejs-redis-nginx-example.dns --format '{{ .host }}'
```
